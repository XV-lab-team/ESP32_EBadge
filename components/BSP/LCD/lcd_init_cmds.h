...
/*
 * ST77916 QSPI 初始化：HD18004C18 厂商表（1.8 寸 360×360，阿里供应商，TFT_eSPI#3915）。
 * 未试过：VoCat/1.5 寸（同属 0xF0=0x28 但 B0=0x69、无 0x76）、Arduino st77916_180、
 * 乐鑫组件默认（实验 T：多行间隔彩点，仍未整圆）。
 * 本表：0xF0=0x28 + 0x76=0x0F + B0=0x52；栅极 0x80=0x48 且行号与 IDF 默认相同（0x04 起）；
 * 源极页 C0=0x65（IDF 是 0x22，180 是 0x12）。
 * 0x4C 不用供应商的「当前窗口填」：改成官方屏外 1 行 kick。末尾 16-bit + 开显示。
 * 不抄 MADCTL 0x08，避免和驱动 RGB 打架。
 */
