...
> **已知：等 DIM（如 50 50 50）不是白，三色不平衡，偏差待定。** 见第 6 节。不要为此改电流档。  
...
- 不要在未测偏差前把 `r==g==b` 当成「白」；见第 6 节

---

## 6. 已知问题：RGB 三色不平衡（2026-08-25，偏差待定）

用户观察：AW9523 共阳 RGB 的 **R / G / B 视觉亮度不平衡**。  
`vio_rgb n 50 50 50`（三路 DIM 相同）时，灯**不是白色**，有可察觉色偏。

| 项目 | 现状 |
|------|------|
| 现象 | 等 DIM（例：50 50 50）≠ 白 |
| 偏差数值 | **待定**（未测哪路偏亮/偏暗，也未测校正曲线） |
| 电流档 | 仍是 `ledmode_isel = 3`，与本问题分开；先不要为了「更白」去改档 |
| 固件 | `io_virtual_rgb_set` / `vio_rgb` 仍把 r/g/b **原样**写 DIM。样式文件 `[cal]` 的 `gain_r/g/b` 只作用于 `ledplay`（当前默认 100/100/100） |

接手注意：

1. 这是 **硬件/灯珠观感** 问题，不是脚表接错。脚表仍按第 2 节。
2. 以后做白平衡时，只应在业务层做 **R/G/B 比例或查找表**，不要改 AW9523 寄存器逻辑，不要动电流档 0/1。
3. 校正前请用户再确认：偏红 / 偏绿 / 偏蓝，以及用哪组灯、哪个 DIM 测。测完把数值补进本节。
